...
export default function CartPage() {
    ...
    const handleCheckout = async () => {
        ...
        try {
            // 1. Limpiamos el carrito en el Backend
            await api.delete('/cart/clear');

            // 2. Sincronizamos items (sin usar any)
            await Promise.all(
                cart.map((item) =>
                    api.post('/cart/add', {
                        productId: item.id,
            ...
            if (response.status === 201) {
                ...
                // 4. LIMPIEZA Y REDIRECCIÓN A LA PÁGINA DE ÉXITO
                // No llamamos a clearCart() aquí porque la página de éxito 
                // lo hace al montarse para asegurar que el usuario vea el cambio.
            ...
            if (axios.isAxiosError(error)) {
                // Manejo de errores de stock insuficiente del backend
        ...
        <div className="max-w-4xl mx-auto px-6 py-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                <div className="lg:col-span-2 space-y-6">
                    {cart.map((item) => (
                        <div key={item.id} className="flex flex-col sm:flex-row items-center gap-6 bg-neutral-950 p-6 rounded-[2rem] border border-neutral-900 shadow-xl group hover:border-red-600/30 transition-all duration-300">
                    ...
                    <button onClick={clearCart} className="text-[10px] font-black uppercase tracking-widest text-neutral-600 hover:text-red-500 transition-colors pl-2">
